...
# )
# @pytest.mark.django_db
# def test_courses_filter_id(ids, status_code, client, course_factory, student_factory):
#     courses = course_factory(_quantity=5)
#     for course in courses:
#         students = student_factory(_quantity=6)
#         course.students.set(students)
#
#     url = '{url}?{filter}={value}'.format(url=reverse("courses-list"), filter='id', value=ids)
#     response = client.get(url)
#
#     assert response.status_code == status_code
#     if response.status_code == HTTP_200_OK:
#         assert response.data[0]["id"] == ids
